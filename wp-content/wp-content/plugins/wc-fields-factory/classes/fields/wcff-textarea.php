...
class wcff_field_textarea extends wcff_field {
	...
	function render_wcff_setup_fields( $type = "wccpf" ) { ob_start(); ?>
		...
		<tr>
			<td class="summary">
				...
				<p class="description"><?php _e( 'Default value for this Text Area', 'wc-fields-factory' ); ?></p>
			</td>
			<td>
		...
		<tr>
			...
			<td>
				<div class="wcff-field-types-meta" data-type="text" data-param="rows">
					<input type="text" id="wcff-field-type-meta-rows" value="4" />
				</div>
			</td>
		</tr>				
	...
	function render_product_field( $field ) { 
		...
		<?php if( has_action('wccpf/before/field/rendering' ) && has_action('wccpf/after/field/rendering' ) ) : ?>
			...
			<textarea class="wccpf-field" name="<?php echo esc_attr( $field["name"] . $name_index ); ?>" placeholder="<?php echo esc_attr( $field["placeholder"] ); ?>" rows="<?php echo esc_attr( $field["rows"] ); ?>" maxlength="<?php echo esc_attr( $field["maxlength"] ); ?>" wccpf-type="text" wccpf-pattern="mandatory" wccpf-mandatory="<?php echo $field["required"]; ?>" <?php echo $readonly; ?> ><?php echo esc_html( $field["default_value"] ); ?></textarea>
	...
	function render_admin_field( $field ) { ob_start(); 
		...
		<p class="form-field <?php echo esc_attr( $field['name'] ); ?>_field ">
			...
			<textarea name="<?php echo esc_attr( $field['name'] ); ?>" class="wccaf-field short" id="<?php echo esc_attr( $field['name'] ); ?>" rows="2" cols="20" placeholder="<?php echo esc_attr( $field['placeholder'] ); ?>" wccaf-type="text" wccaf-pattern="mandatory" wccaf-mandatory="<?php echo $field["required"]; ?>"><?php echo esc_attr( $field['value'] ); ?></textarea>
			<?php 
		...
		<div class="form-field">
			<label for="<?php echo esc_attr( $field['name'] ); ?>"><?php echo wp_kses_post( $field['label'] ); ?><?php echo ( isset( $field["required"] ) && $field["required"] == "yes" ) ? ' <span>*</span>' : ''; ?></label>
			<textarea name="<?php echo esc_attr( $field['name'] ); ?>" class="wccaf-field short" id="<?php echo esc_attr( $field['name'] ); ?>" rows="5" placeholder="<?php echo esc_attr( $field['placeholder'] ); ?>" wccaf-type="text" wccaf-pattern="mandatory" wccaf-mandatory="<?php echo $field["required"]; ?>"><?php echo esc_attr( $field['value'] ); ?></textarea>
