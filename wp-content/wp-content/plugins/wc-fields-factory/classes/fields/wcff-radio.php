...
class wcff_field_radio extends wcff_field {
	...
	function render_wcff_setup_fields( $type = "wccpf" ) { ob_start(); ?>
		...
		<tr>
			<td class="summary">
				...
				<p class="description"><?php _e( 'Enter default selected option', 'wc-fields-factory' ); ?></p>
			</td>
			<td>
	...
	function render_product_field( $field ) { 
		...
		<?php if( has_action('wccpf/before/field/rendering' ) && has_action('wccpf/after/field/rendering' ) ) : ?>
			...
			foreach ( $choices as $choice ) {
				...
				<li><label><input type="radio" class="wccpf-field" name="<?php echo esc_attr( $field["name"] . $name_index ); ?>" value="<?php echo esc_attr( trim( $key_val[0] ) ); ?>" <?php echo $attr; ?> wccpf-type="radio" wccpf-pattern="mandatory" wccpf-mandatory="<?php echo $field["required"]; ?>" <?php echo $readonly; ?> /> <?php echo esc_html( trim( $key_val[1] ) ); ?></label></li>
			<?php } ?>		
			</ul>
	...
	function render_admin_field( $field ) { ob_start(); 
		...
		foreach ( $choices as $choice ) {
			...
			$check_list .= '<li><input type="radio" class="wccaf-field select short" name="'. esc_attr( $field["name"] ) .'" value="'. esc_attr( trim( $key_val[0] ) ) .'" '. $attr .' wccaf-type="radio" wccaf-pattern="mandatory" wccaf-mandatory="'. $field["required"] .'" /> '. esc_html( trim( $key_val[1] ) ) .'</li>';
		...
		<fieldset class="form-field <?php echo esc_attr( $field['name'] ); ?>_field">
			<legend><?php echo wp_kses_post( $field['label'] ); ?><?php echo ( isset( $field["required"] ) && $field["required"] == "yes" ) ? ' <span>*</span>' : ''; ?></legend>
			<ul class="wc-radios">
			<?php echo $check_list; ?>			
			</ul>
		<?php 
